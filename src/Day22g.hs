...
stepTimers s = s
  { manaPool      = manaPool s + if rechargeTimer s > 0 then 101 else 0
  , bossHp        = bossHp s - if poisonTimer s > 0 then 3 else 0
...
advance s = go (stepTimers $ applyDifficulty s) ho
  where
    go :: GameState -> (GameState -> [GameState]) -> [GameState]
    go s k
      | playerDead s = []
      | bossDead s   = [s]
      | otherwise    = k s

    ho :: GameState -> [GameState]
    ho s1 = do 
      sp <- availableSpells s1
      turns sp
      where
        turns :: Spell -> [GameState]
        turns spell = go (stepTimers (applySpell spell s1)) bossTurn
          where
            bossTurn :: GameState -> [GameState]
            bossTurn s2 = go (bossAttack s2) return
